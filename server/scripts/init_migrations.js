...
const sqls = [
  ...
  `CREATE TABLE IF NOT EXISTS ${config.MYSQL_DATABASE}.state (id INT AUTO_INCREMENT PRIMARY KEY, name VARCHAR(50) NOT NULL, value LONGTEXT NOT NULL, createdAt TIMESTAMP DEFAULT CURRENT_TIMESTAMP);`,
  `CREATE TABLE IF NOT EXISTS ${config.MYSQL_DATABASE}.users (id INT AUTO_INCREMENT PRIMARY KEY, name VARCHAR(50) NOT NULL, phone VARCHAR(50) NOT NULL, password VARCHAR(50) NOT NULL, isActive BOOLEAN DEFAULT false, refreshToken VARCHAR(500), role VARCHAR(100) DEFAULT 'user', CHECK (role IN ('user', 'admin')), createdAt TIMESTAMP DEFAULT CURRENT_TIMESTAMP);`,
  ...
  `CREATE TABLE IF NOT EXISTS ${config.MYSQL_DATABASE}.menu (id INT AUTO_INCREMENT PRIMARY KEY, name VARCHAR(50) NOT NULL, description LONGTEXT NOT NULL, photoUrl VARCHAR(50) NOT NULL, category VARCHAR(50) NOT NULL, subCategory VARCHAR(50) NOT NULL, unit VARCHAR(50) NOT NULL, price INT NOT NULL, scale INT NOT NULL, isScaled BOOL DEFAULT false, createdAt TIMESTAMP DEFAULT CURRENT_TIMESTAMP);`,
